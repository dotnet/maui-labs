...
With **Local provider** or **Cloud** selected, the **RECORDING** section shows **Record** to capture real streaming or non-streaming responses, or **Stop** to stop capturing without discarding the tape. Each completed interaction auto-saves to `FileSystem.CacheDirectory/chat-playground.autosave.json`. Once a tape exists, **Clear** (trash can) empties it and **Save** copies it atomically to `FileSystem.AppDataDirectory/chat-playground.recording.json`. Clearing while recording keeps capture enabled for subsequent responses. The OS may clear app cache; save recordings you need to keep. Debug recordings can contain private content; review them before sharing.

Select the third **Recording** client to play the active recording without invoking either model, even if a provider is unavailable. When the tape is empty, the section shows only **Load** (folder) to choose a JSON file; loading validates it and atomically replaces the cached tape. When it contains interactions, **Clear** empties it before a different file can be selected, **Play** renders the next interaction and its original request, and **Save** remains available. **Rewind** (skip-to-start) appears after playback begins and returns to the first interaction; **Play** disappears when the tape is exhausted. Each icon has a visible action label as well as an accessible description and tooltip. Live response, tool, generation, and chat-composer controls are disabled while Recording is selected. Switch to Local or Cloud to continue chatting with the replayed conversation history and your existing live settings. **Clear conversation** keeps the recording and restarts replay from the beginning. The low-level replay client still strictly matches message and option data against recorded `DataContent` when called through `IChatClient`.
