...
cart.MapPost("/items", (GardenStore store, AddToCartRequest request) =>
    {
        if (store.GetProduct(request.Sku) is null)
            return Results.NotFound(new ProblemDetails { Detail = $"Product '{request.Sku}' does not exist." });
        if (request.Quantity <= 0)
            return Results.BadRequest(new ProblemDetails { Detail = "Quantity must be greater than zero." });
        return Results.Ok(store.AddToCart(request));
    })
...
cart.MapPut("/items/{sku}", (
    ...
    {
        if (store.GetProduct(sku) is null)
            return Results.NotFound(new ProblemDetails { Detail = $"Product '{sku}' does not exist." });
        if (request.Quantity < 0)
            return Results.BadRequest(new ProblemDetails { Detail = "Quantity cannot be negative." });
        return Results.Ok(store.SetCartQuantity(sku, request.Quantity));
    })
...
orders.MapPost("/", (GardenStore store) =>
    store.GetCart().Items.Count == 0
        ? Results.BadRequest(new ProblemDetails { Detail = "Cannot check out an empty cart." })
        : Results.Ok(store.Checkout()))
    .WithName("checkout")
...
reviews.MapPost("/", (GardenStore store, CreateReviewRequest request) =>
    {
        ...
        if (request.Rating is < 1 or > 5)
            return Results.BadRequest(new ProblemDetails { Detail = "Rating must be between 1 and 5." });
        var created = store.CreateReview(request);
        return Results.Created($"/products/{created.Sku}/reviews", created);
    })
