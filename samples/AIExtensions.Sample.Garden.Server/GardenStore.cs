...
namespace AIExtensions.Sample.Garden.Server;
...
public sealed class GardenStore
{
    ...
    private static IEnumerable<Product> Seed() => GardenProductFixtures.Catalog;
    ...
    public Cart GetCart()
    {
        ...
        foreach (var (sku, qty) in _cart)
        {
            if (!_products.TryGetValue(sku, out var p))
                continue;
            items.Add(new CartItem(p.Sku, p.Name, p.Price, qty, p.Price * qty, p.Emoji, p.ImageUrl));
        }
    ...
    {
        _cart.AddOrUpdate(r.Sku, r.Quantity, (_, existing) => existing + r.Quantity);
        return GetCart();
    }
    ...
    {
        var review = new Review(Guid.NewGuid().ToString("n")[..8], r.Sku, r.Rating, r.Comment, DateTimeOffset.UtcNow);
