namespace AIExtensions.Sample.ChatPlayground.Features.Embeddings.Models;

public sealed record ImportedDocument(string Id, string Name, int CharacterCount, DateTimeOffset ImportedAtUtc)
{
    public string Details => $"{CharacterCount} characters - imported {ImportedAtUtc.ToLocalTime():g}";
}

internal sealed record StoredDocument(string Id, string Name, string Text, DateTimeOffset ImportedAtUtc)
{
    public ImportedDocument Summary => new(Id, Name, Text.Length, ImportedAtUtc);
}
