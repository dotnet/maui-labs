...
internal static class ChatRecordingSerializer
{
    ...
    public static void AssertEqual(int interaction, JsonObject expected, JsonObject actual) =>
        Compare(interaction, "$.messages",
            expected["messages"] ?? throw new InvalidDataException("The recorded request has no messages."),
            actual["messages"] ?? throw new InvalidDataException("The replay request has no messages."));
        ...
        if (expected is JsonArray expectedArray && actual is JsonArray actualArray)
        {
            if (expectedArray.Count != actualArray.Count)
                throw new ChatRecordingMismatchException(
                    ...
                    JsonValue.Create(expectedArray.Count), JsonValue.Create(actualArray.Count));
