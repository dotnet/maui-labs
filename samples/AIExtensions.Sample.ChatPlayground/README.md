...
Selecting **Replay** replaces the composer with **Find chats**, **Restart**, **Play all**, and **Next turn**. Find chats opens a dialog: typing filters saved text and titles locally, and **Search** asks the selected embedding provider for semantic matches. Each result shows a matching turn excerpt; selecting it opens that recording and plays its full chat into view without contacting a chat model. **Import file** is a separate button in the dialog; it validates external JSON, copies it into the library as a new chat, and automatically plays it. A cancelled picker or invalid file preserves the active chat. The current chat is restored and replayed on app startup. Restart clears the displayed transcript and rewinds without deleting the recording; Next turn advances one interaction, and Play all restarts from the beginning. After playback, select a real client to continue. Live sending is blocked during partial playback or after Restart. **Stop** cancels playback and lets you retry. Export, Restart, Play all, and Next turn are disabled for an empty chat; Find chats and Import file remain available.
...
On iOS and Mac Catalyst, `Microsoft.Maui.Essentials.AI.NLEmbeddingGenerator` uses Apple's local English NaturalLanguage sentence embeddings by default (available on older Apple OS versions than `AppleIntelligenceChatClient` chat). On Android, text filtering works without an embedding provider. To enable Azure OpenAI `text-embedding-3-small` on any platform, configure the separate `AI:EmbeddingDeploymentName` and tap **Enable Azure search** in Find chats. The confirmation explains that saved chat text and search terms will be sent to Azure; it is **off by default** and never activated as a fallback for failed local embeddings. **Use private search** stops future Azure calls. Apple and Azure vectors are never compared to each other, even if their dimensions match. A provider error explicitly falls back to keyword results; a cloud request is made only after consent and an explicit Search or subsequent New.
