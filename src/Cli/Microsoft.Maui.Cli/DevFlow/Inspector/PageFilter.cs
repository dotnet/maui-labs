...
/// first: (1) the agent-selected Shell current page; (2) the selected Tab/FlyoutItem whose id
/// encodes the target page type; (3) otherwise the
...
internal static class PageFilter
{
    public static HashSet<string> InactivePageIds(List<ElementInfo>? roots)
    {
        ...
        // Hidden tab bars have no selected Tab marker; the agent still knows Shell.CurrentPage.
        ElementInfo? active = null;
        bool confident = false;
        var selectedPages = pages.Where(page => page.IsSelected).ToList();
        var tabbed = pages
        ...
        else if (tabbed.Count > 0 && tabbed[0].s > 0)
        {
            active = tabbed[0].p;
            confident = true;
        }
        ...
        // Only competing pages can be inactive: navigation ancestors/descendants and disjoint
        // flyout/detail panes can be displayed together. When we only GUESSED the active page, never hide a
        ...
        foreach (var p in pages)
        {
            ...
            if (IsDescendantOf(p.Id, active.Id) || IsDescendantOf(active.Id, p.Id)) continue;
            var pageBounds = Abs(p);
            var activeBounds = Abs(active);
            if (pageBounds is { Width: > 0, Height: > 0 } && activeBounds is { Width: > 0, Height: > 0 }
                && (pageBounds.X + pageBounds.Width <= activeBounds.X
                    ...
                    || activeBounds.Y + activeBounds.Height <= pageBounds.Y))
                continue;
