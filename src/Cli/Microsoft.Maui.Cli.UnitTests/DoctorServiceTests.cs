...
public class DoctorServiceTests
{
	...
	[Fact]
	public async Task TryFixAsync_WithAutoFixableCommand_ReturnsTrue()
	{
		// Arrange
		...
		// Act
		var result = await service.TryFixAsync(fix);
	...
	[Fact]
	public async Task TryFixAsync_WithNonAutoFixable_ReturnsFalse()
	{
		// Arrange
		var service = new DoctorService();
		var fix = new FixInfo
	...
	[Fact]
	public async Task TryFixAsync_WithInvalidCommand_ReturnsFalse()
	{
		// Arrange
		var service = new DoctorService();
		var fix = new FixInfo
		{
			...
			Description = "Test fix",
			AutoFixable = true,
			Command = "/invalid/path/to/nonexistent/command"
		};
	...
	[Theory]
	[InlineData("echo test", "echo", new[] { "test" })]
	[InlineData("dotnet workload install maui", "dotnet", new[] { "workload", "install", "maui" })]
	[InlineData("xcode-select -s /path/to/xcode", "xcode-select", new[] { "-s", "/path/to/xcode" })]
	[InlineData("  echo    test  ", "echo", new[] { "test" })]
	public void ParseCommand_WithVariousInputs_ParsesCorrectly(string command, string expectedFile, string[] expectedArgs)
	{
		// Act
	...
	[Theory]
	[InlineData("\"C:\\Program Files\\tool.exe\" --flag", "C:\\Program Files\\tool.exe", new[] { "--flag" })]
	[InlineData("\"C:\\Program Files\\tool.exe\"", "C:\\Program Files\\tool.exe", new string[] { })]
	public void ParseCommand_WithQuotedPaths_ParsesCorrectly(string command, string expectedFile, string[] expectedArgs)
	{
		// Act
		var (fileName, args) = DoctorService.ParseCommand(command);
	...
	[Theory]
	[InlineData("--path \"C:\\Program Files\\foo\" --name bar", new[] { "--path", "C:\\Program Files\\foo", "--name", "bar" })]
	[InlineData("--flag1 value1 --flag2 \"quoted value\"", new[] { "--flag1", "value1", "--flag2", "quoted value" })]
	public void TokenizeArguments_WithQuotedArguments_TokenizesCorrectly(string input, string[] expected)
	{
		// Act
		var result = DoctorService.TokenizeArguments(input);
	...
	[Fact]
	public async Task RunAllChecksAsync_WithXcodeCompatibilityCheck_IncludesCheck()
	{
		// Arrange
		var fakeApple = new FakeAppleProvider
		{
			HealthChecks = new List<HealthCheck>
			{
				new HealthCheck
				{
					...
					Status = CheckStatus.Ok,
					Message = "All SDK packs compatible"
				}
			}
		};
		...
		// Assert
		Assert.Contains(report.Checks, c => c.Category == "apple" && c.Name == "Xcode Compatibility");
	}
}
