...
and **whole pre-built screens** that must be used as-is (a checkout/payment surface, a monthly orders
...
app-specific styles/controls/screens live in the app. The model only *selects* registered names and
...
| **Control** (custom control) | An app composite control (frame, layers, watermark, custom input). | A node `type` with a `props` object. | type name + alias | Compose it into the generated tree. |
| **Screen** (full custom screen) | A whole app-owned surface/screen. | The `present_screen` tool, or a `Screen` node. | type name + alias | Gather declared inputs, then hand off. |

The escalating idea: **Style** tweaks an existing control; **Control** replaces one node with a
bespoke control; **Screen** replaces the whole surface with an app-owned screen.
...
public sealed class SessionUi(GenerativeUiRegistry ui)
{
    ...
    {
        ui.AddScreen<AdminOrdersScreen>("AdminOrdersScreen",
            "All-orders admin screen. Only when signed in as a manager.");
        ui.AddControl<BulkPriceEditor>("BulkPriceEditor",
            "Edit many product prices at once. Manager only.");
    }

    public void OnSignOut()
    {
        ui.Remove("AdminOrdersScreen");
...
The MVP is **imperative** — `AddStyle` / `AddControl<TControl>` / `AddScreen<TScreen>`. Later, a
source generator (or reflection over `[UiControl]`-style attributes and `ResourceDictionary`
entries) can register directly from XAML; every path produces the same registry entries, so
discovery and validation don't care how something was registered.
...
options.Ui.AddStyle("primary",
    "Emphasized call-to-action — the single main action on a screen.",
    appliesTo: ["Button"]);                 // resourceKey defaults to "primary"

options.Ui.AddStyle("danger",
    "Destructive action (delete, remove, clear). Signals irreversible intent.",
    appliesTo: ["Button"],
    resourceKey: "DangerButtonStyle");      // override: match the real XAML x:Key

options.Ui.AddStyle("Brand",
    "Brand accent color for emphasis text and badges.",
    appliesTo: ["Label", "Badge"],
    resourceKey: "BrandAccentColor");       // a Color resource
```

- **`appliesTo`** is the list of control types the token is valid on (built-in DSL node types such as
  `Button`/`Entry`/`Label`, or a registered control alias). A node matches if its control **is that
  type or derives from it** — so a registered `PrimaryButton : Button` still matches `appliesTo:
  ["Button"]` without listing it explicitly.
...
### 3.2 Controls (custom controls)

A control is an app composite control exposed as a **node type**. It is identified by its **control
...
options.Ui.AddControl<StarRating>("StarRating",
...
- **Props** are a light list: each has a `name`, a `Description`, and an optional `Editable` flag
  ...
  the MVP.
- **Creation** is via **DI** (`ActivatorUtilities.CreateInstance<TControl>`), so controls can take
  constructor services. The model never provides code.
- **In the DSL**, a control is an ordinary node whose `props` object supplies values (literals,
...
- The model may reference a control by its **alias** (`ProductImage`) or its **full type name** —
...
### 3.3 Screens (full custom screens)

A screen is a **whole app-owned surface** the model hands off to. Unlike a control, the model does
**not** compose its internals; it selects the screen and supplies declared inputs. Screens
**self-load bulk data** through their own VM/services (the same API/HttpClient), so the model needn't
pass large payloads. A screen is identified by its **type** (created via DI) and an **alias**, with a
...
- **Presented** via the **`present_screen`** tool, which takes over the canvas, or embedded as a
  `Screen` node inside a larger generated layout. Full-canvas is the MVP default; region/overlay/
  persistent hosting is future.
- **Inputs** use the same light schema as control props; the model supplies only these.
...
```jsonc
{ "screen": "MonthlyOrdersReport", "inputs": { "month": "2026-06", "verbosity": "detailed" } }
```
...
- `list_ui_capabilities()` → styles, controls, screens (names + aliases + descriptions).
- `describe_control(name)` → prop list.
- `describe_screen(name)` → input list + description.
...
- The **valid node-type set** = built-ins + registered controls + `Screen`; the **valid style set**
  = base + registered styles; the **valid screen set** = registered screens — read from the
...
- **Inflation order:** built-in → registered control/screen → unknown ⇒ graceful placeholder
  (never throw). Control `props` and screen `inputs` are validated against their declared lists
  before the control or screen is created.

## 6. Binding & state

- **Control props:** literal, one-way `bind` (into `data`), or two-way `key` (into the form) for
  ...
  control inputs arrive through that generic tree.
- **Screens:** receive resolved `inputs`; they own their VM and load their own bulk data via services
  (same app auth/HttpClient), so the model passes parameters, not datasets.
- All screen creation marshals to the UI thread during inflation/hosting.

## 7. Security & trust

- **No model-authored code or markup.** Controls and screens are compiled app C# created via DI. The
  model only chooses registered names and supplies inputs validated against declared lists.
- **Screens self-load via app services** (app auth/HttpClient), never from model-supplied URLs.
- **Input validation** happens before any control or screen is created; invalid props degrade to placeholders.
...
```csharp
// startup
options.Ui.AddStyle("hero", "Oversized primary CTA.", appliesTo: ["Button"], resourceKey: "HeroButtonStyle");
options.Ui.AddControl<ProductImage>("ProductImage", "…", props: [ /* … */ ]);
options.Ui.AddScreen<CheckoutScreen>("CheckoutScreen", "…");

// later (e.g. after manager sign-in), via the injected GenerativeUiRegistry
ui.AddScreen<AdminOrdersScreen>("AdminOrdersScreen", "All-orders admin screen. Manager only.");
// …and on sign-out
ui.Remove("AdminOrdersScreen");
```

### 9.2 Model uses a registered control
...
### 9.3 Model hands off to a full screen

```jsonc
// present_screen — checkout takes over the canvas; the screen loads the cart itself
{ "screen": "CheckoutScreen", "inputs": {} }
```
...
5. **Overriding built-ins:** allow apps to replace a built-in style/control by name, or keep
   built-ins immutable?
6. **Screen data contracts:** how does a screen express "I self-load X" vs. "the model must give me Y"
   so the model knows what to gather? (MVP: prose in the description + declared `inputs`.)
7. **Screen hosting:** MVP presents screens full-canvas. When do we need region/overlay/persistent
   hosting, and who owns layout/z-order then?
8. **Runtime changes mid-render:** if a control/screen is removed while a surface using it is on
