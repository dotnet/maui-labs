...
- **Unknown identifiers pass through.** An identifier DevFlow does not recognize is lowercased and returned unchanged rather than rejected or coerced onto another platform, so a newer or out-of-tree agent stays usable with an older CLI. `DevFlowPlatform.IsKnown` distinguishes the two cases. Note the deliberate asymmetry with the schema: the `enum` in `schemas/agent-status.json` is a *closed, additively grown* list of the platforms DevFlow ships identity for, so an agent on an unlisted platform functions at runtime but will not validate against the schema until its identifier is added there.
- **Filters compare canonical identity.** When both an agent's platform and a requested filter resolve to platforms DevFlow knows, canonical equality is authoritative and nothing else is considered — a `linux` filter must never match an agent reporting `Tizen (Linux) 8.0`. A substring fallback applies only when one side is unrecognized, so partial filters such as `tiz` keep working.
