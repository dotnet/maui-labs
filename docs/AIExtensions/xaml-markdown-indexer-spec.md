...
When a document's root type is `Shell`, it is parsed to (a) list the top-level
content it hosts, (b) identify the app's home screen, and (c) associate explicitly
declared `ShellContent` routes with their hosted pages in generated catalog metadata.
The Shell parse recognizes `TabBar`, `Tab`, `FlyoutItem`, and `ShellContent`.
**Routes are never rendered in semantic Markdown** because a user does not perceive
them; the generated metadata exists only to connect semantic pages to navigation.
...
The explicit `Route` segments on the containing `TabBar`, `Tab`, `FlyoutItem`, and
`ShellContent` are combined into an absolute route and associated with that hosted
page in the aggregate catalog. Generated or implicit routes are not inferred.
...
- It exposes:
  ...
  - `Pages` — the list of every indexed page, as `IndexedPage` records, **ordered alphabetically by
    page name**. Each entry carries the page's name, file path (or none), Markdown, and its explicit
    fully qualified CLR type identity, and all explicit absolute `ShellContent`
    routes that were resolved.
  ...
  - `EntryPageTypeName` — the home/entry screen's fully qualified CLR type
    identity, used to disambiguate equal simple class names.
...
- **`IndexedPage`** — an indexed page: `Name`, fully qualified `TypeName`,
  `FilePath` (nullable), `Markdown`, `Routes`, and the first-route convenience
  property `Route` (nullable generated navigation metadata).
...
`Route="products"` does not appear in semantic Markdown; it is emitted only on the aggregate
`IndexedPage.Route` metadata.
...
page name `ProductDetailPage`. The `ProductDetailPage` entry also carries the explicit
