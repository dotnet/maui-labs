...
**Zero findings**: If all reviewers return zero findings, skip Step 4. Instead call `add_comment` with: "✅ Expert Code Review: {N} independent reviewers found no issues. Methodology: {N}-model adversarial consensus." — where **{N} is the number of reviewers that actually completed** (3 normally, 2 in the fallback mode from Step 2), never a hardcoded 3. In 2-reviewer mode, append " (1 reviewer failed to complete — reduced coverage, no tiebreaker)" so the clean bill does not overstate how many models cleared the PR.
...
2. **Methodology**: "{N} independent reviewers with adversarial consensus" — **{N} = reviewers that actually completed** (3 normally, 2 in the Step 2 fallback), never a hardcoded 3. If only 2 completed, write "2 independent reviewers (1 failed — reduced coverage, no tiebreaker)" instead of claiming 3.
...
End the summary with (substitute **{N}** = the number of reviewers that actually completed — 3 normally, 2 in the Step 2 fallback — never a hardcoded 3):
```
> Generated by Expert Code Review · {N} independent reviewers with adversarial consensus
```
